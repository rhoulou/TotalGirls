...
/**
 * CloudStream 3 provider for FikFapCams live cams.
 *
 * Scrapes fikfapcams.com directly from the phone (no addon server), mirroring
 * the logic in bobs/fikfapcams/viewer.php:
 *
 *   * Room list -> https://fikfapcams.com/api/front/v2/models?primaryTag=
 ...
 */
class FikfapcamsProvider : MainAPI() {
    override var mainUrl = BASE_URL
    override var name = "FikFapCams"
    ...
    override suspend fun search(query: String): List<SearchResponse>? {
        ...
        println("FikFapCams search '$query' -> ${models.size} results")
    ...
    private fun Model.toSearchResponse(provider: FikfapcamsProvider): SearchResponse =
    ...
    private suspend fun fetch(url: String): String {
        ...
        for (attempt in 0 until 3) {
            try {
                ...
                if (res.isSuccessful) {
                    println("FikFapCams GET ${res.okhttpResponse.request.url} -> ${res.okhttpResponse.code} (${res.text.length}B)")
                    return res.text
                }
                if (res.okhttpResponse.code == 429) { // rate limited -> longer pause
                    println("FikFapCams GET ${res.okhttpResponse.request.url} -> 429 (rate limited)")
                    delay(RATE_LIMIT_PAUSE_MS)
                    continue
                }
                println("FikFapCams GET ${res.okhttpResponse.request.url} -> ${res.okhttpResponse.code}")
                return ""
            } catch (e: Exception) {
        ...
        println("FikFapCams request failed: $lastError")
        return ""
    }
    ...
    /** Minimum gap between fikfapcams.com requests, shared across providers. */
    ...
    companion object {
        private const val BASE_URL = "https://fikfapcams.com"
        ...
        // Home rows mirroring the viewer's category dropdown (bobs/fikfapcams/viewer.php).
